...
const LogsWindow = ({ token, canDrag = false }: { token: string, canDrag?: boolean }) => {
    ...
    const fetchlogs = async (nextPage: number) => {
        ...
        try {
            ...
            const res = await fetch(
                `${process.env.NEXT_PUBLIC_ENDPOINT || "https://202.10.34.67:5050"}/api/logs-proctored-user/${token}?page=${nextPage}&paginationLimit=7`,
                {
                    headers: {
    ...
    const insertNewLogsFromSocket = async (count: number) => {
        ...
        try {
            const jwt = await session();
            const res = await fetch(
                `${process.env.NEXT_PUBLIC_ENDPOINT || "https://202.10.34.67:5050"}/api/logs-proctored-user/${token}?page=1&paginationLimit=${count}`,
                {
                    headers: {
                        Authorization: `Bearer ${jwt}`,
                    },
