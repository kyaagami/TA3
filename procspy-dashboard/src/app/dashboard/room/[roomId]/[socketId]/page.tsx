...
export default function Page() {
    ...
    const fetchUserInfo = async (token: string) => {
        try {
            ...
            const res = await fetch(`${process.env.NEXT_PUBLIC_ENDPOINT || 'https://192.168.43.85:5050'}/api/proctored-user/${token}`, {
    ...
    const fetchSessionResult = async (token: string) => {
        try {
            const jwt = await session();
            const res = await fetch(`${process.env.NEXT_PUBLIC_ENDPOINT || 'https://192.168.43.85:5050'}/api/session-result-token/${token}`, {
                headers: { Authorization: `Bearer ${jwt}` },
            });
            if (res.ok) {
                const data = await res.json();
