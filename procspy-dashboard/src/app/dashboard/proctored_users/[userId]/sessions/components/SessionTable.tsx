...
const SessionTable = () => {
    ...
    const fetchSessions = async (nextPage: number) => {
        try {
            ...
            const res = await fetch(`${process.env.NEXT_PUBLIC_ENDPOINT || 'https://0.0.0.0:5050'}/api/sessions/${userId}?page=${nextPage}&paginationLimit=20`, {
                headers: {
    ...
    const handleSessionState = async (token: string, state: string) => {
        try {
            ...
            const res = await fetch(`${process.env.NEXT_PUBLIC_ENDPOINT || 'https://0.0.0.0:5050'}/api/session/update-status-proctor/${token}/${state}`, {
                headers: {
