...
const ProctorTable = () => {
    ...
    const fetchProctor = async (nextPage: number) => {
        try {
            ...
            const res = await fetch(`${process.env.NEXT_PUBLIC_ENDPOINT || 'https://0.0.0.0:5050'}/api/users?page=${nextPage}&paginationLimit=20`, {
                headers: {
    ...
    const deleteProctor = async (id: string) => {
        try {
            ...
            const response = await fetch(`${process.env.NEXT_PUBLIC_ENDPOINT || 'https://0.0.0.0:5050'}/api/user/${id}`,
                {
                    method: "DELETE",
                    headers: {
    ...
    const addProctor = async ({username, name, email, password}:{username: string, name: string, email: string, password: string}) => {
        ...
        try {
            const jwt = await session()
            const response = await fetch(`${process.env.NEXT_PUBLIC_ENDPOINT || 'https://0.0.0.0:5050'}/api/register`,
                {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
    ...
    const editProctor = async ({ id, username, name, email }: { id: string, username: string, name: string, email: string }) => {
        ...
        try {
            const jwt = await session()
            const response = await fetch(`${process.env.NEXT_PUBLIC_ENDPOINT || 'https://0.0.0.0:5050'}/api/user`,
                {
                    method: "PUT",
                    headers: {
                        "Content-Type": "application/json",
    ...
    const activateUser = async (id: string) => {
        ...
        try {
            const token = await session();
            const res = await fetch(`${process.env.NEXT_PUBLIC_ENDPOINT || 'https://0.0.0.0:5050'}/api/activate-user/${id}`, {
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${token}`,
                },
