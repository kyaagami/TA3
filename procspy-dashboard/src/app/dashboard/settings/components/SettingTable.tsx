...
const SettingTable = () => {
  ...
  const fetchSettings = async (nextPage: number) => {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_ENDPOINT || 'https://0.0.0.0:5050'}/api/global-settings?page=${nextPage}&paginationLimit=12`, {
        headers: {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const token = await session();
      const response = await fetch(`${process.env.NEXT_PUBLIC_ENDPOINT || 'https://0.0.0.0:5050'}/api/global-setting`, {
        method: "PUT",
        headers: {
