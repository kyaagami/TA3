...
const setSessionStatus = async (state: string, token: string): Promise<boolean> => {
  try {
    const response = await fetch(`${process.env.ENDPOINT || 'https://192.168.68.225:5050'}/api/session/update-status/${token}/${state}`, {
      method: "GET",
      headers: {
...
export const handleSocketConnection = async (socket: Socket) => {
  ...
  const saveLog = async (flagKey: string, token: string, attachment: Record<string, any> = {}) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.ENDPOINT || 'https://192.168.68.225:5050'}/api/save-log`, {
        method: "POST",
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Secret ${process.env.SECRET || "SECRET"}`
  ...
  const saveFile = async (file: File): Promise<string | null> => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.ENDPOINT || "https://192.168.68.225:5050"}/api/storage`, {
        method: "POST",
        headers: {
          'Authorization': `Secret ${process.env.SECRET || "SECRET"}`
        },
